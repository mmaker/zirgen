...
void verifyValidity(ReadIopVal& iop,
  ...
  Val result = compute_poly(evalU, globals, mix, polyMix);
  if (kDebug)
...
VerifyInfo verify(ReadIopVal& iop, size_t po2, const CircuitInterface& circuit) {
  ...
  auto computePoly = [&](llvm::ArrayRef<Val> u,
                         ...
                         Val polyMix) { return circuit.compute_poly(u, out, accumMix, polyMix); };
...
void verifyV3(ReadIopVal& iop, size_t po2, const CircuitInterfaceV3& circuit) {
  ...
  auto computePoly = [&](llvm::ArrayRef<Val> u,
                         llvm::ArrayRef<Val> out,
                         llvm::ArrayRef<Val> accumMix,
                         Val polyMix) { return circuit.computePolyExt(u, out, accumMix, polyMix); };
